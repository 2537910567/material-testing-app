...
def perf_dwg_01_xlarge_text_only(results):
    ...
    if prof_result:
        ...
        m_profile.notes = ("reason=" + str(prof_result.strategy_reason) +
                           ", has_chinese_path=" + str(_has_chinese_chars(dwg_path)))
    else:
        m_profile.strategy = ""
        m_profile.notes = "reason=N/A"
    results.add_perf(m_profile)

    strategy = prof_result.strategy if prof_result else "text_only"
    print("    Profile -> strategy=" + str(strategy) +
          " (" + str(prof_result.strategy_reason if prof_result else "?") + ")")
    ...
    if conv_result and isinstance(conv_result, dict):
        ...
        m_conv.notes = ("png=" + str(png_count) +
                        ", text_chars=" + str(text_len) +
                        ", error=" + str(conv_result.get("error", "None")))
    results.add_perf(m_conv)

    has_text = (conv_result and isinstance(conv_result, dict) and
                 len(conv_result.get("text", "")) > 10)
    passed = prof_result is not None and (has_text or m_conv.output_count > 0)

    details = {
        "file_size_mb": file_size_mb,
        "strategy": strategy,
        "strategy_reason": (prof_result.strategy_reason if prof_result else ""),
...
def perf_dwg_02_large_boundary(results):
    ...
    if prof:
        ...
        m1.notes = "reason=" + str(prof.strategy_reason)
    else:
        m1.strategy = ""
    results.add_perf(m1)
    ...
    print("    Profile -> strategy=" + str(strategy) +
          " (" + str(prof.strategy_reason if prof else "?") + ")")
    ...
    if conv and isinstance(conv, dict):
        ...
        m2.notes = ("text_len=" + str(len(conv.get("text", ""))) +
                     ", err=" + str(conv.get("error", "None")))
    results.add_perf(m2)

    passed = prof is not None
    details = {"strategy": strategy,
               "reason": prof.strategy_reason if prof else "",
               "png_count": m2.output_count,
               "peak_mb": m2.peak_memory_mb}
    results.add("TP-DWG02", "7.2MB DWG [" + str(strategy) + "]",
...
def perf_dwg_03_medium_reduced(results):
    ...
    results.add("TP-DWG03", "3-4MB DWG [" + str(strategy) + "]",
...
def perf_dwg_04_batch_throughput(results):
    ...
    details = {
        ...
        "success_rate": (round(m.output_count / len(dwg_paths) * 100, 1)
                          if dwg_files else 0,
    }
...
def perf_dwg_05_chinese_path(results):
    ...
    _, m_direct = measure_operation("DWG05:direct_from_path",
    ...
    with t_copy:
        ...
        if ascii_path:
            ...
            m_ascii_op.notes = ("copy_cost=" + str(round(ascii_copy_time, 0)) + "ms, " +
...
def perf_pdf_01_xlarge_profile(results):
    ...
    if prof:
        ...
        m.notes = ("pages=" + str(m.output_count) +
...
def perf_pdf_02_xlarge_text(results):
    ...
    has_text = (content and isinstance(content, dict) and
...
def perf_pdf_04_medium_full_pipeline(results):
    ...
    if os.path.exists(small_path):
        _, sm = measure_operation("PDF04:small_compare",
                                   extract_pdf_content, small_path)
    ...
    if m.duration_ms > 0 and m.output_count > 0:
        chars_per_sec = m.output_count / (m.duration_ms / 1000)
    details = {"text_chars": m.output_count,
    ...
    results.add("TP-PDF04", "18.4MB PDF 完整管线 (" + str(file_size_mb) + "MB)",
               passed, m.duration_ms, details=details)
    rate_str = str(round(chars_per_sec, 0)) if chars_per_sec > 0 else "?"
    dur_s = round(m.duration_ms / 1000, 1)
    status = "PASS" if passed else "FAIL"
    print("    [" + status + "] - " + str(dur_s) + "s, " +
...
def main():
    ...
    perf_dwg_01_xlarge_text_only(results)
    perf_dwg_02_large_boundary(results)
    perf_dwg_03_medium_reduced(results)
    perf_dwg_04_batch_throughput(results)
    perf_dwg_05_chinese_path(results)

    # Part B: PDF 性能测试
    print("\n  ━━━━ PDF Performance Tests ━━━━")
    perf_pdf_01_xlarge_profile(results)
    perf_pdf_02_xlarge_text(results)
    perf_pdf_03_xlarge_render30(results)
    perf_pdf_04_medium_full_pipeline(results)
...
def _save_markdown_report(results):
    ...
    for pm in results.perf_measurements:
        ...
        row = ("| `" + str(pm.test_id) + "` | " + str(pm.operation) +
                ...
                str(pm.strategy or "-") + " | " + str(int(pm.duration_ms)) +
